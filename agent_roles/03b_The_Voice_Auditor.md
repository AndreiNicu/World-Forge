...
**Shape the bid to the declared posture.** In an `adversarial`/`indifferent`/`mixed` world the bid is a *demand against resistance*. In a `deferential` or `predatory` world that scenario tests nothing — the world yields by design — so build it instead as an **ask running the other way**: an NPC working `{{user}}` for something, using the vectors and voiced phrasings from Master Design Section 6's `Works on {{user}} by`. That is the scene where a `predatory` world's real failure modes (rescue-from-consequence, tell-rule violation) become visible, and a bid built the standard way will pass while the world is broken.

**When any character's function includes harming, using, or deceiving, add a "cruelty" scenario** — the character at the point of doing the thing they do — so Step 3L has material. Placed where mitigation would be most tempting: the beat where the target is most sympathetic, or where the character has the clearest opening to hesitate.

The lull, temptation, cruelty, and cross-axis scenarios are additional classes with their own trigger conditions. The bid class above has no trigger condition — it always applies.
...
- **No rescue from consequence.** The fourth reflex, and the only one that fires in a world where nothing can physically touch `{{user}}` — so run it on every `deferential` and `predatory` world, where the other three legitimately pass. After `{{user}}` does something the world says should cost them, does the cost actually land, or did the model quietly dissolve it? The recognizable forms: the ask gets **softened** before it is made (the terrible favor becomes a mild one); the **target turns out to have deserved it**, retroactively; an NPC **breaks frame to check on `{{user}}`** or to reassure them it wasn't so bad; the deed **doesn't take** (the order is not carried out, the harm is undone off-screen); or the next scene opens on word that **it worked out fine**. Where Master Design Section 6's losables are `moral` or `epistemic`, this check is the primary one and the other three are noise — grade it 🔴 Critical, because a world whose entire stake is moral weight and whose moral weight evaporates has no stake at all.
- **Tell-rule integrity (where Master Design Section 6 declares one).** In an `opaque` world, scan the generated narration for warnings the drafts did not authorize: a smile that doesn't reach the eyes, "something calculating," a manipulative line delivered *smoothly* / *silkily* / *carefully*, or any narrator framing that marks an appeal as a play. Each is a violation even when the prose is good — especially when the prose is good, since that is how it survives review. In a `visible-but-tempting` world, check the inverse failure: the temptation written as weak, so that seeing through it costs `{{user}}` nothing. This check is worth running before the others in a manipulation world; a broken tell rule invalidates the scene as a test of anything else.
...
- 🔴 **Critical** — an NPC concedes a load-bearing thing purely because `{{user}}` asked; or a stated attempt against real resistance is rendered as accomplished fact; or the turn writes `{{user}}`'s actions or decisions; or, in a world whose stakes are `moral`/`epistemic`, the consequence of what `{{user}}` did was dissolved.
- 🟠 **High** — a manufactured rescue with no antecedent; the arc's named opposition behaves as though it were not opposition; adversity with no traceable source; a tell-rule violation in an `opaque` world.
...
**Posture-specific weighting.** Which reflex is worth your attention depends on the declared posture, and running the generic list flat wastes the audit on a `predatory` or `deferential` world:

| Declared posture | Where the failure actually lives |
|---|---|
| `adversarial` / `indifferent` / `mixed` | Yield, auto-success, rescue. The standard three. Also check that a world declaring death/defeat on the table actually permits a scene to be lost — a desperate fight that cannot be lost is the reflex wearing a costume. |
| `deferential` | Yield and auto-success **correctly pass** — the world defers, that is the design. The live checks are the named exception (does the one thing that doesn't bow actually not bow, or has the room's deference leaked onto it?) and rescue-from-consequence. |
| `predatory` | The first three pass by design and mean nothing here. **Rescue-from-consequence and the tell rule are the entire audit.** Build the bid scenario as an *ask* rather than a demand — an NPC working `{{user}}` for something — and check whether the appeal landed as warmth, whether the ask stayed as monstrous as authored, and whether the cost survived into the next beat. |
...
### Step 3L — Antagonist Fidelity (runs when any character's function includes harming, using, or deceiving)

The sibling of Step 3K, on the cast side. Step 3K asks whether the world opposes `{{user}}`; this asks whether a character written to be cruel is *rendered* cruel. The model reliably supplies mitigation the drafts never authored — a flicker of doubt before the cruel line, interiority reframing the cruelty as pain, a hesitation, a softening in the final beat, an implication they don't really mean it.

Generate a scene that puts the character at the point of doing the thing they do, and check:

- **Unauthored mitigation.** Is every softening element traceable to drafted material — the card's `description`, its stated interiority, a CHARACTER_STATE line? Mitigation you cannot trace is the finding. Apply the counterfactual: would the drafts equally permit the *un*-softened rendition? If the character came out sympathetic and nothing compelled it, the sympathy is the model's, not the world's.
- **The two hard cases.** Where the drafts state the character **feels nothing** about what they do, check for guilt appearing anyway. Where the drafts state they **love their target and do it anyway**, check for the love being written as conflict — hesitation, a divided moment, the ask half-retracted. These are the two configurations the model will not hold unaided, and they are usually the point of the character.
- **Self-justification present and coherent.** Does the character operate from a frame in which they are the reasonable one? An antagonist rendered as arbitrarily evil is the opposite failure and it is the one that follows from over-correcting this check — flag it with equal weight.
- **The final-beat drift.** Check the last exchange specifically. The most common shape of this bug is a scene that plays the cruelty straight for four beats and then lands on a softening note, because the model resolves toward comfort at the end of its output. A scene that was correct until its last line still fails.

Severity: 🔴 Critical where the mitigation reverses the character's stated function (a manipulator written as reluctant, a predator written as conflicted). 🟠 High for unauthored interiority that does not reverse the function. 🟡 Medium for final-beat drift alone, or for a mitigation the drafts merely fail to forbid (⚠️ NOT BINDING).

Diagnosis routes to the Architect: name the card and the specific missing element — an unstated interiority (§5 antagonist-fidelity block asks for it explicitly, including the "feels nothing" and "loves them anyway" cases), or a `system_prompt` prohibition that is generic ("do not soften her") where it needs to be concrete. If the mitigation reproduces across *every* antagonist in the world, it is engine-side instead — surface it to the Prompt Engineer as a Phase 5 note.

Worlds with no character whose function includes harm skip Step 3L.
...
| The cost of what {{user}} did was softened, walked back, or reported as fine (Step 3K, rescue-from-consequence) | Stance directive missing its rescue-from-consequence prohibition, or the arc names no `moral`/`epistemic` losable for the cost to attach to. In a `predatory` world this is the primary failure — check the stance bullet before anything else |
| Narration tells the player an NPC is insincere in an `opaque` world (Step 3K, tell rule) | Tell rule not carried into the ARC_STATE / SANDBOX_STATE Tonal Mandate (Architect §8.A tell-rule block), or carried abstractly rather than as the concrete prohibition list. Not a card-side fix — the narrator is the preset + Tier 3, not the character |
| Antagonist rendered with mitigation the drafts never authored (Step 3L) | Card `description` states no interiority (Architect §5 antagonist-fidelity block), or the `system_prompt` prohibition is generic rather than naming this character's specific mitigation risk. If it reproduces across every antagonist, it is engine-side — surface to Prompt Engineer |
| Antagonist rendered as arbitrarily evil, with no coherent self-justification (Step 3L inverse) | Over-correction. The card carries prohibitions without a stated frame in which the character is the reasonable one — direct the Architect to author the self-justification, not to add more prohibitions |
...
[State the declared posture first, and audit against the weighting table for it — a flat run of all four reflexes on a `deferential` or `predatory` world reports three meaningless passes and misses the real failure.

One row per bid scenario. Columns: arc / the ask or attempt / did the NPC yield-because-asked (with the traceable reason if it conceded) / was a stated attempt auto-succeeded / was there a manufactured rescue / **did the cost of what {{user}} did survive the scene** / **tell-rule integrity where one is declared** / did the stance directive bind / any railroading (agency seizure, narrated defeat, no-win construction, sourceless adversity). Record ⚠️ NOT BINDING where the scene passed but the counterfactual shows the drafts equally permit the deferential rendition. Close with the world-side vs. engine-side split: which findings route to the Architect (named ARC_STATE / SANDBOX_STATE stance entry) and which route to the Prompt Engineer as a Phase 5 note (uniform across the whole matrix = preset-side).]

## Antagonist Fidelity (Step 3L — worlds with a character whose function includes harm)
[One row per such character. Columns: the cruelty scenario / mitigation present (yes/no) / traceable to a drafted line (quote it) or unauthored / the two hard cases where they apply (stated "feels nothing" → did guilt appear? stated "loves them and does it anyway" → was the love written as conflict?) / self-justification present and coherent / final-beat drift. Note the inverse failure separately: any antagonist rendered as arbitrarily evil with no coherent frame. Omit this section, noting "N/A — no character whose function includes harm," where Step 3L does not apply.]
...
- [ ] **Bid scenario present in every arc (every world, both modes), shaped to the declared posture — a demand against resistance in `adversarial`/`indifferent`/`mixed`, an NPC's ask *of* `{{user}}` using Section 6's voiced vectors in `deferential`/`predatory`. At least one bid states an attempt against resistance in the form a real player writes it**
- [ ] **Cruelty scenario present for every character whose function includes harming, using, or deceiving; OR confirmed the world has none and Step 3L skipped**
...
- [ ] **Step 3K run on every arc (no exemptions), weighted to the declared posture: no yield-because-asked, no auto-succeeded attempts, no manufactured rescues, **no rescue-from-consequence** (the primary check in `deferential`/`predatory` worlds, where the other three pass by design), tell-rule integrity where one is declared, stance directive exercised — each judged against the counterfactual, not against how smooth the scene read. Inverse half also verified: no agency seizure, no narrated `{{user}}` defeat, no no-win construction, no sourceless adversity. Findings split world-side (Architect) vs. engine-side (Prompt Engineer, Phase 5).**
- [ ] **Step 3L run where the world has characters whose function includes harm: no mitigation untraceable to a drafted line; the "feels nothing" and "loves them anyway" configurations hold; self-justification present and coherent; no final-beat softening. Inverse verified — no antagonist rendered as arbitrarily evil without a frame.**
