...
| Section 3 — `Posture Toward {{user}}` | regenerate (always) | — | Rides Section 3, and every field in it is computed against a specific protagonist: *who does not defer* is a stance toward **this** person, *what {{user}} can lose* names **their** attachments and standing, and the off-the-table boundary is the user's line for **this** character. A new protagonist voids all four. Carrying the block would be worse than dropping it — a stale posture reads as authored and quietly aims the world's opposition at someone who is no longer in it. Re-elicit at Step 4 question 7 alongside the rest of Section 3, at the Interviewer's Section 3 depth. **The `Default posture` enum itself may be *offered* as a starting suggestion** ("the source world was `mixed` — still?") since a world's general hardness often survives a protagonist swap; the other three fields are always re-asked. Rebaseline keeps the whole block (same `{{user}}`, same world) |
