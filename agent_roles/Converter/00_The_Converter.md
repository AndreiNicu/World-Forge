...
7. **The new protagonist (`{{user}}`).** This is the heart of a Reframe conversion. Ask the same depth the Interviewer asks at Phase 0 Section 3 — wound, hidden layer, contradiction, power and limits, physical description, voice, **and the Posture Toward `{{user}}` block**. Push hard here.

   On the posture block specifically: it regenerates in full because every field is computed against a particular protagonist — who does not defer is a stance toward *this* person, what is losable names *their* attachments, the boundary is the user's line for *this* character. Run the Interviewer's four questions (Section 3, "Posture toward {{user}}"), with one shortcut available: you may **offer** the source's `Default posture` as a starting suggestion — "the source world was `mixed`; still `mixed` for the new protagonist?" — because a world's general hardness often does survive a protagonist swap. Do not offer the other three fields as carry-overs; ask them fresh. And surface the coupling where it bites: if the conversion is a **power-tier shift** (mortal → deity, outsider → insider), say so out loud, because a posture written for a powerless protagonist is exactly backwards for a powerful one, and this is the field users most often forget to re-think after a tier change. The new protagonist is being grafted into an existing world; if the new protagonist is thin, the graft fails. Reuse the Interviewer's Section 3 questioning approach (see `agent_roles/00_The_Interviewer.md` Section 4 — SECTION 3: THE PROTAGONIST).
...
- **Section 3 (The Protagonist):** *Always new content.* Write the new protagonist from the user's Step 4 question 7 answers, **including the `Posture Toward {{user}}` block** — all four fields plus the boundary, written against the new protagonist. Do not carry across the old protagonist's content. If the user accepted the source's `Default posture` enum as a starting suggestion, that is fine and needs no marker (an enum value is not source content); the other fields are their fresh answers. Never leave the block empty on the grounds that it will be re-elicited downstream — the Refiner treats an empty posture as a gap and an unstated posture ships as deference.
...
| Section 3 — `Posture Toward {{user}}` | regenerate (always) | — | Rides Section 3, and every field in it is computed against a specific protagonist: *who does not defer* is a stance toward **this** person, *what {{user}} can lose* names **their** attachments and standing, and the off-the-table boundary is the user's line for **this** character. A new protagonist voids all four. Carrying the block would be worse than dropping it — a stale posture reads as authored and quietly aims the world's opposition at someone who is no longer in it. Re-elicit at Step 4 question 7 alongside the rest of Section 3, at the Interviewer's Section 3 depth. **The `Default posture` enum itself may be *offered* as a starting suggestion** ("the source world was `mixed` — still?") since a world's general hardness often survives a protagonist swap; the other fields are always re-asked. **A power-tier shift is the case that most often changes the enum** — a protagonist promoted to deity, insider, or absolute authority turns a `mixed` world into a `deferential` or `predatory` one, and the cast's relationship to them inverts. Surface that explicitly rather than offering the old value. Manipulation vectors and the tell rule regenerate with the block: the vectors are appeals aimed at a *specific* protagonist's attachments ("do it for your wife" needs a wife), so they never carry. Rebaseline keeps the whole block (same `{{user}}`, same world) |
...
| Section 3 — `Posture Toward {{user}}` | regenerate (always) | **keep** — same `{{user}}`, same world, so all four fields still hold. Carry per Step D: the source seed's block as the base, with any posture-change revision applied in place. **One rebaseline-specific prompt worth making:** if the source has *no* posture block (a world built before the posture contract), the divergence scan will not surface it, because it is absent from both the seed and any revision. Ask for it once during the fold-in — a consolidation is the cheap moment to add it, and shipping the rebuild without it means the rebuilt world plays deferential no matter how the old one played |
