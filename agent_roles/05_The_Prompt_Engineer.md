...
Master Design Section 6 carries a **`Posture Toward {{user}}`** block with a declared `Default posture`. **Default-include the `protagonist_jeopardy` optional block (§5a) whenever that posture is `adversarial`, `indifferent`, or `mixed`** — and also when it is `deferential` but Section 6 names a force that does not defer, which it should in every world. **Always include it when the posture is `predatory`**: that world's entire stake is the fourth failure mode (rescue from consequence) and no other block defends it. The only world that omits the block is one deliberately built with no friction anywhere.

**Read the harm classes on Section 6's losables before authoring.** Where they are `moral` or `epistemic` — complicity, self-image, being deceived and acting on it — the fourth prohibition is the block's load-bearing content and the first three are close to decorative, because a world built on those stakes is usually one where `{{user}}` cannot be physically or materially touched at all. Weight the block accordingly rather than shipping the generic ordering.
...
#### Antagonist-integrity block hint (all modes — read the cast, not the genre)

**Default-include the `antagonist_integrity` optional block (§5a) in any world with a character whose function includes harming, using, or deceiving** — an antagonist, a predator, a manipulator, an institution with a face. Like `protagonist_jeopardy`, it is **not genre-keyed**: it addresses a disposition the model brings, and it fires in a cozy world with one manipulative in-law exactly as in a grimdark one. Read Master Design Sections 7 and 8 (character foundations, NPC roster) for the cast's functions rather than inferring from Section 1's tone.

Predict the matching failure modes by name: **"the model supplies hesitation, doubt, or redemptive interiority the drafts never authored"** and **"the narrator marks an insincere character's lines with a tell, warning the reader"**. Two authoring requirements: the block stays world-agnostic (this world's antagonists and its declared tell rule live in the cards and Tier 3), and the **counter-guardrail against flat villainy is not optional content** — an antagonist with no coherent frame is the over-correction, and Pass 2 checks for it exactly as it does for the jeopardy block's.

Weight this block heavily when Master Design Section 6 declares a **tell rule** (`opaque` especially) or manipulation vectors: those worlds run on appeals landing as sincere, and the narrator-annotation reflex breaks them on the first scene.
...
- [ ] If `protagonist_jeopardy` block included: content leads with the co-author frame (positively stated, before any prohibition), names the trained satisfy-the-user disposition explicitly, gives all **four** failure modes as imperatives (no yield-because-asked / a stated attempt is an attempt / no manufactured rescue / **no rescue from consequence**, with moral and epistemic costs named as real stakes), **contains the counter-guardrail with equal weight** (never seize `{{user}}`'s agency, never narrate their defeat for them, never engineer a no-win scene, never manufacture adversity the world's interests do not supply — opposition, not punishment), and defers the world's specific refusals to the active ARC_STATE / SANDBOX_STATE stance directive as binding authority. A block missing the counter-guardrail fails this check outright — it is not a soft flag; the block without it produces a worse bug than its absence.
- [ ] Master Design Section 6 declares a `Posture Toward {{user}}` with a posture warranting the block (`adversarial` / `indifferent` / `mixed` / `predatory`, or `deferential` with a named non-deferring force): the `protagonist_jeopardy` block is included and enabled — or the omission carries an explicit Step 4 justification that is not merely "this world is gentle." **`predatory` admits no omission justification**, and where Section 6's losables are `moral`/`epistemic` the fourth prohibition must be present and weighted, not merely listed.
- [ ] If `antagonist_integrity` block included: content leads with the fidelity frame, names the mitigation reflex explicitly, covers no-unauthored-mitigation (including the stated "feels nothing" and "loves them and does it anyway" cases), the final-beat rule, and the narrator-does-not-annotate-sincerity rule, **and contains the counter-guardrail against flat villainy with equal weight** (antagonists operate from a coherent frame; authored warmth and complexity are rendered in full; what is forbidden is inventing complexity as an apology). Missing counter-guardrail = fail, same standing as the jeopardy block's.
- [ ] The world's cast includes a character whose function is harming, using, or deceiving: the `antagonist_integrity` block is included and enabled — or the omission carries an explicit Step 4 justification. Mandatory where Master Design Section 6 declares a tell rule of `opaque`, since the narrator-annotation reflex breaks that world on the first scene.
...
- [ ] **`protagonist_jeopardy` included and enabled per the protagonist-jeopardy block hint (Master Design Section 6 posture is `adversarial`/`indifferent`/`mixed`/`predatory`, or `deferential` with a named non-deferring force) — or the omission justified in the Block Selection Rationale (Step 4) on grounds other than the world being gentle; `predatory` admits no omission. Where included: all four prohibitions present (the fourth — no rescue from consequence — weighted where Section 6's losables are moral/epistemic), the counter-guardrail (no agency seizure, no narrating {{user}}'s defeat, no no-win scenes, no manufactured adversity) present with equal weight, and no world-specific content**
- [ ] **`antagonist_integrity` included and enabled where the cast contains a character whose function is harming, using, or deceiving (mandatory under an `opaque` tell rule) — or the omission justified in Step 4. Where included: no-unauthored-mitigation with both hard cases, the final-beat rule, the narrator-does-not-annotate-sincerity rule, and the counter-guardrail against flat villainy at equal weight; no world-specific content**
