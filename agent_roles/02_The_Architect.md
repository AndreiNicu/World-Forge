...
- `agent_roles/02a_ARC_STATE_Examples.md` — two full ARC_STATE worked examples, when authoring an `ARC_STATE` entry (Section 8.A; arc mode only)
...
Two full worked ARC_STATE examples — the same world at two arcs with opposite registers (a grim coercive Arc 1 and a wholesome redemption Arc 4) — live in **`agent_roles/02a_ARC_STATE_Examples.md`**. Open that file when you want a concrete model of the two-subsection format before authoring an entry. It is illustration only; the binding rules are the structure spec above and the Editor hard-fail below.
