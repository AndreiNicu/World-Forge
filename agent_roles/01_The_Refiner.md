...
- **Afterward (characters with intimate presence — when Section 8 is in scope):** record what this character does in the ten minutes after intimacy, in both registers — the ordinary bodily business (cleanup, urinating, water, showering or not) and what they do with the other person (stay/leave, hold/turn away, talk/go quiet/joke/sleep) — plus the one aftermath behavior that signals a change of state. This is arc-agnostic Tier 2 substrate feeding the Intimacy Architect's Entry 6b. It is authored substrate rather than prose texture because the model terminates intimate scenes at climax by default; the substrate is what the aftermath prohibition reaches for. Where an arc changes the pattern itself (not merely how it is performed), note it as a per-arc register delta.
- **Embodied baseline (characters with intimate presence — when Section 8 is in scope):** record the physical facts of this body as they bear on intimate contact — age and what the body has lived through (births, injury, surgery, illness, labor, athletics, world-specific alteration), build and scale, arousal and recovery mechanics, the particulars of the acts this world contains, and the trajectory (what is different now than a decade ago). This is arc-agnostic Tier 2 substrate feeding the Intimacy Architect's Entry 3 Half A. **Record it in observable register, not clinical vocabulary** — the Master Design text propagates into entries that reach the model's context, and clinical phrasing there produces anatomy-lecture prose at runtime. **Record both directions** — what has become harder *and* what has become easier, more certain, or no longer performed. **Record the declared valence** for any culturally loaded attribute (age, size, weight, scars, disability): neutral fact / advantage / charged. Each carries two opposed trained defaults, so an unstated valence is a coin flip, not a neutral — log unstated ones to `UNRESOLVED_QUESTIONS.md`. Carry this for *every* intimate character, including default-bodied ones; if the seed filled it in only for older characters, flag the gap to `UNRESOLVED_QUESTIONS.md` rather than letting the stock default stand for the rest.
- **Physical dyad (per pairing, when Section 8 is in scope):** for each recurring intimate pairing — `{{user}}` first — record the height/reach, build/strength, stamina/recovery, age-gap, and experience differentials **and what each materially costs or changes** (positions, pacing, who adjusts, what each partner notices), plus **anatomical fit — what each act actually costs this pair** (what needs preparation and what does not, what is comfortable, what takes work, what a partner does not have to accommodate). Where Section 3 records `{{user}}`'s intimate embodiment, compute the `{{user}}` dyads against it rather than against a default, and surface any contradiction between the two rather than silently picking one. This is pairing-level substrate that no per-character field can hold; it feeds the Intimacy Architect's Entry 7 Half B. Required wherever a real differential exists; the asymmetry must run in both directions (each partner has what the other lacks). If the seed states differentials as bare facts with no stated consequence, or in one direction only, log it to `UNRESOLVED_QUESTIONS.md`.
