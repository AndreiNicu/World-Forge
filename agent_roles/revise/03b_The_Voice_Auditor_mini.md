...
9. **Run Step 3K (protagonist jeopardy) on state and posture scopes.** The parent runs Step 3K on every arc unconditionally; you run it on the slice. Include a **bid scenario** — `{{user}}` asking for, taking, or attempting something the scene has no reason to grant — whenever the revision recalibrates `ARC_STATE`/`SANDBOX_STATE`, touches a character's stance toward `{{user}}`, or is a posture change. Apply the parent's full check: no yield-because-asked, a stated attempt stays an attempt, no manufactured rescue, the stance directive actually binds, **and the inverse half** — no agency seizure, no narrating `{{user}}`'s defeat, no no-win construction, no sourceless adversity.
   ...
   Three things specific to the revision case. First, a posture-change revision is the highest-risk place for the **over-correction**: the user asked for a harder world, the mini-Architect sharpened the stance bullet, and the model now writes `{{user}}`'s losses for them. Weight the inverse half accordingly — it is the failure this revision is most likely to have introduced. Second, when the revision was prompted by a "the world is too soft" complaint, reproduce the user's own excerpt (Step R3.5.2) as a bid fixture and check whether the edit actually changed the outcome; a stance bullet that reads better but produces the same concession is ⚠️ NOT BINDING, not a pass. Third, **shape the bid to the declared posture and weight the reflexes per the parent's posture table** — on a `deferential` or `predatory` world, build the bid as an NPC's ask *of* `{{user}}` and audit rescue-from-consequence and the tell rule; running the standard demand-against-resistance bid there reports three meaningless passes. Route uniform-across-everything findings to the preset (`protagonist_jeopardy` block / jailbreak clause 2b) per the parent's world-side vs. engine-side split.

10. **Run Step 3L (antagonist fidelity) on character scopes touching a harm-function character.** When the revision adds or recalibrates a character whose function includes harming, using, or deceiving — `tier2_new_character`, `tier2_character_voice_calibration`, or any card edit touching their mandates — include a **cruelty scenario** and apply the parent's Step 3L: is every softening element traceable to a drafted line, do the "feels nothing" / "loves them and does it anyway" configurations hold, is the self-justification coherent, does the final beat drift soft? A voice-calibration revision on an antagonist is the classic place for mitigation to creep back in, because sharpening a voice frequently means rewriting the passage that stated the interiority. Check the inverse too: a revision asked to make a character "harder" that produced arbitrary cruelty with no frame is the over-correction, and routes back to the Architect for a self-justification rather than more prohibitions.
