...
│   ├── 05b_Preset_Resync.md          ← Preset Resync Mode procedure (split from 05; loaded only under /worldforge resync-preset)
...
- **Three postures.** The default (spec Sections 1–7) is the fresh-fragment start. The **improvement posture** (spec Section 8) brainstorms *changes to an already-consolidated world*: it reads the world's `World_Seed.md` + Conversion Manifest and diverges on what to add, rework, or deepen — respecting the world's spine (an idea that flips World Mode, swaps the protagonist, or overturns the core concept is flagged as reframe-conversion territory, not a rebaseline improvement). This is what the Converter's **`--then-brainstorm`** flag invokes (#10): after a rebaseline consolidates a revised world, the Brainstormer explores improvements, writes its notes, and the chain continues into the Interviewer's seed-revision posture, which reads those notes as *proposals*. The **revision-diagnostic posture** (spec Section 9) serves the post-launch *pre-articulation* case — something feels off in a shipped world but the user can't name what to revise. Invoked by `/worldforge revise --brainstorm` (#6), it reads `Drafts/Master_Design.md` read-only, diagnoses divergently (the four domain lenses as its diagnostic vocabulary, the "where does it sag" move as its primary instrument), and writes one primary concern (+ candidate future concerns) to `Brainstorm_Notes.md`; the Reviser reads that as captured intent. It enforces the same Section 1 / 11a / World Mode bright line and bounces an out-of-scope diagnosis rather than dressing it as a revision.

**Boundaries:** the Brainstormer is upstream of the pipeline (and, in the revision-diagnostic posture, upstream of the Reviser), not a phase of it — it does not advance the Pipeline State Ledger, invoke downstream agents, or produce any first-class pipeline artifact (`Brainstorm_Notes.md` stays informal in all three postures). It is a front porch. Its one downstream coupling is the revision-diagnostic posture feeding the Reviser a located concern (#6); beyond that it touches no other architectural principle.

See the **BRAINSTORM** section of `workflows/world-forge.md`.
...
| `agent_roles/05b_Preset_Resync.md` (Preset Resync Mode procedure, split from 05) | `workflows/world-forge.md` PRESET RESYNC section and `agent_roles/05_The_Prompt_Engineer.md` (manifest + TWO ENTRY MODES pointer to the companion; resync re-runs §5f self-validation + foundational rules) — all define the `/worldforge resync-preset` operation |
